...
export abstract class ArcherRouter {
  ...
  public static swapCallParameters(
    ...
    const etherIn =
      trade.inputAmount.currency === Ether.onChain(ChainId.MAINNET);
    const etherOut =
      trade.outputAmount.currency === Ether.onChain(ChainId.MAINNET);
    // the router does not support both ether in and out
    invariant(!(etherIn && etherOut), 'ETHER_IN_OUT');
    invariant(!('ttl' in options) || options.ttl > 0, 'TTL');
    invariant(
      'ethTip' in options &&
        options.ethTip?.currency === Ether.onChain(ChainId.MAINNET),
    );
