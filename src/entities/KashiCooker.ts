...
import { ChainId, WNATIVE } from '@digitalnative/standard-protocol-sdk-test';
...
export default class KashiCooker {
  ...
  bentoDepositCollateral(amount: BigNumber): KashiCooker {
    const useNative =
      this.pair.collateral.address === WNATIVE[this.chainId].address;
  ...
  addAsset(amount: BigNumber, fromBento: boolean): KashiCooker {
    ...
    if (fromBento) {
      ...
      const useNative =
        this.pair.asset.address === WNATIVE[this.chainId].address;
