...
const localeFromChainId = Number(localStorage.getItem(storageConstants.KEY_FROM_CHAIN_ID + process.env.NEXT_CBRIDGE_APP_ENV));
// const chainArray = process.env.NEXT_CBRIDGE_APP_ENV === "TEST" ? [3, 5, 97] : [1, 56, 137, 42161, 100, 66];
let defaultFromChainId;
if (localeFromChainId) {
  defaultFromChainId = localeFromChainId;
} else {
  defaultFromChainId = process.env.NEXT_CBRIDGE_APP_ENV === "TEST" ? 3 : 1;
}
const initialState: ISliceState = {
  allToken: {},
  selectedIndex: Number(localStorage.getItem(storageConstants.KEY_SELECTED_INDEX)) || 0,
  fromChainId: defaultFromChainId,
  toChainId: localStorage.getItem(storageConstants.KEY_TO_CHAIN_ID + process.env.NEXT_CBRIDGE_APP_ENV)
    ? Number(localStorage.getItem(storageConstants.KEY_TO_CHAIN_ID + process.env.NEXT_CBRIDGE_APP_ENV))
    : Number(process.env.NEXT_CBRIDGE_APP_BSC_ID),
  loading: false,
  isReject: false,
  tvl: 0,
  ...
  waitingNum: 0,
  defaultSize: 50,
  safeMargin: process.env.NEXT_CBRIDGE_APP_ENV === "TEST" ? 20 * 60 * 1000 : 1 * 60 * 60 * 1000,
  isChainShow: false,
  chainSource: "from",
  transferConfig: {
    chains: [],
...
export const switchChain = async (id, isMobile, atoken) => {
  ...
  if (process.env.NEXT_CBRIDGE_APP_ENV === "TEST" || isMobile) {
    return;
  }
  try {
