...
export function useYggPrice(swrConfig: SWRConfiguration = undefined) {
  const { chainId } = useActiveWeb3React();
  const { data } = useSWR(
    chainId && chainId === ChainId.MAINNET ? 'yggPrice' : null,
    () => getYggPrice(),
    swrConfig,
  );
  return data;
...
export function useTruPrice(swrConfig: SWRConfiguration = undefined) {
  const { chainId } = useActiveWeb3React();
  const { data } = useSWR(
    chainId && chainId === ChainId.MAINNET ? 'truPrice' : null,
    () => getTruPrice(),
    swrConfig,
  );
  return data;
...
export function useMphPrice(swrConfig: SWRConfiguration = undefined) {
  const { chainId } = useActiveWeb3React();
  const { data } = useSWR(
    chainId && chainId === ChainId.MAINNET ? 'mphPrice' : null,
    () => getMphPrice(),
    swrConfig,
  );
  return data;
...
export function useLiquidityPositions(
  ...
  const { data } = useSWR(
    shouldFetch
      ? ['liquidityPositions', chainId, JSON.stringify(variables)]
      : null,
...
export function useTransactions(
  ...
  const { data } = useSWR(
    shouldFetch
      ? ['transactions', chainId, query, JSON.stringify(variables)]
      : null,
...
export function useTokenPairs(
  ...
  const { data } = useSWR(
    shouldFetch
      ? ['tokenPairs', chainId, query, JSON.stringify(variables)]
      : null,
