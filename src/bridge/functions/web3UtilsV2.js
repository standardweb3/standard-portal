...
import { getCurChainInfo } from './bridge';
...
function getBatchWeb3Data(rpc, list) {
  return new Promise((resolve, reject) => {
    ...
    for (const obj of list) {
      if (obj.data) {
        batch.add(
          web3.eth.call.request({ data: obj.data, to: obj.to }, 'latest'),
        );
      } else {
        batch.add(
          web3Fn[obj.property][obj.methods].request(...obj.inputFormatter),
        );
      }
    }
...
async function useBatchWeb3(chainId, list) {
  const rpcArr = getCurChainInfo(chainId).nodeRpcList;
...
export { getWeb3, useWeb3, useBatchWeb3, getContract };
