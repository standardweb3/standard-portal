...
export function useBentoBalances(): BentoBalance[] {
  ...
  const balanceData = useSingleCallResult(boringHelperContract, 'getBalances', [
    account,
    tokenAddresses,
  ]);
  ...
  return useMemo(() => {
    ...
    return tokenAddresses
      .map((key: string, i: number) => {
        ...
        const usd = e10(token.decimals).mulDiv(
          uiData.result[0].ethRate,
          balanceData.result[0][i].rate,
        );
        ...
        return {
          ...
          balance:
            token.address === weth
              ? uiData.result[0].ethBalance
              : balanceData.result[0][i].balance,
          ...
          bento: easyAmount(
            toAmount(full, balanceData.result[0][i].bentoBalance),
            full,
          ),
        };
      })
...
export function useBentoBalance(
  tokenAddress: string,
): {
  value: BigNumber;
  decimals: number;
