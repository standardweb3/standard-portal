...
export interface BaseStrategyWithHasPermitTokenHook
  extends BaseStrategyHook,
    BaseTrait {}
...
const useHasPermitTokenTrait = (
  ...
  const { signatureData, gatherPermitSignature } = useERC20Permit(
    inputValue,
  ...
  const batchExecute = useCallback(
    async (val: CurrencyAmount<Token>) => {
      const method = zapIn
        ? props.general.zapMethod
        : props.general.unzapMethod;
      ...
      try {
        ...
        if (signatureData) {
          ...
          addTransaction(tx, {
            summary: `Approve Inari Master Contract and ${
              zapIn ? 'Deposit' : 'Withdraw'
