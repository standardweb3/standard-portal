...
const useStakeSushiToCreamStrategy = (): StrategyHook => {
  ...
  const approveAmount = useMemo(
    () => (zapIn ? inputValue : cTokenAmountRef.current),
    [inputValue, zapIn],
  );
  ...
  const toCTokenAmount = useCallback(
    async (val: CurrencyAmount<Token>) => {
      ...
      const bal = await zenkoContract.toCtoken(
        CRXSUSHI.address,
        val.quotient.toString(),
      );
  ...
  useEffect(() => {
    ...
    const main = async () => {
      ...
      const bal = await zenkoContract.fromCtoken(
        CRXSUSHI.address,
        balances[CRXSUSHI.address]
          .toFixed()
          .toBigNumber(CRXSUSHI.decimals)
          .toString(),
      );
      setBalances({
        inputTokenBalance: balances[SUSHI[ChainId.MAINNET].address],
        outputTokenBalance: CurrencyAmount.fromRawAmount(
          XSUSHI,
          bal.toString(),
        ),
      });
