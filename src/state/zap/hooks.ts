...
export function useDerivedZapInfo(
  ...
  const isTradingCurrency0 =
    ...
    (currency0?.symbol === WNATIVE[ChainId.MAINNET].symbol &&
      currency === Ether.onChain(ChainId.MAINNET));
  const isTradingCurrency1 =
    currency?.symbol === currency1?.symbol ||
    (currency1?.symbol === WNATIVE[ChainId.MAINNET].symbol &&
      currency === Ether.onChain(ChainId.MAINNET));

  const currencyZeroTrade = useV2TradeExactIn(
    tradeAmount,
    currency0 ?? undefined,
  );
  const currencyOneTrade = useV2TradeExactIn(
    tradeAmount,
    currency1 ?? undefined,
  );
  ...
  const liquidityMinted = useMemo(() => {
    if (pair && totalSupply && currencyZeroOutput && currencyOneOutput) {
      return pair.getLiquidityMinted(
        totalSupply,
        currencyZeroOutput.wrapped,
        currencyOneOutput.wrapped,
      );
    } else {
      return undefined;
    }
  ...
  const allowedSlippage = useUserSlippageToleranceWithDefault(
    DEFAULT_ZAP_SLIPPAGE_TOLERANCE,
  );
